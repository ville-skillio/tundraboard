...
// ---------------------------------------------------------------------------
// PART 4 — Verification Pipeline Issues Found in AI-Generated Code
// ---------------------------------------------------------------------------

// When labels.ts was first generated by the AI assistant, running
// `npm run verify` (scripts/verify.sh) caught the following issues before
// the code ever reached a human reviewer. Each entry records the exact
// location, the type of issue, and which pipeline stage surfaced it.

export const pipelineIssuesFound = [
  {
    stage: "1 — Type check (tsc --noEmit)",
    file: "src/routes/labels.ts",
    line: 34,
    issue: "Property 'workspace_id' does not exist on type 'Label'",
    detail:
      "The AI used snake_case field names matching the raw SQL schema (workspace_id, " +
      "created_at) instead of the camelCase names Prisma exposes (workspaceId, createdAt). " +
      "TypeScript caught all six occurrences as compile errors. " +
      "Fix: renamed all snake_case references to camelCase.",
  },
  {
    stage: "2 — Lint (eslint)",
    file: "src/routes/labels.ts",
    line: 12,
    issue: "ESLint: 'LabelService' is defined but never used (@typescript-eslint/no-unused-vars)",
    detail:
      "The AI imported LabelService at the top of the file but then called the Prisma " +
      "client directly in the route handlers, leaving the import dead. " +
      "Fix: removed the unused import and routed all DB calls through LabelService.",
  },
  {
    stage: "2 — Lint (eslint)",
    file: "src/routes/labels.ts",
    line: 58,
    issue: "ESLint: Promises must be awaited or explicitly ignored (no-floating-promises)",
    detail:
      "The DELETE handler called prisma.label.delete(...) without await — a fire-and-forget " +
      "mutation that returned 204 before the deletion completed. Under concurrent load " +
      "this silently discards database errors. Fix: added await.",
  },
  {
    stage: "4 — Tests with coverage",
    file: "tests/labels.test.ts",
    line: 91,
    issue: "AssertionError: expected 404, received 500",
    detail:
      "GET /labels/:id returned 500 when the label did not exist because the AI omitted " +
      "a null-check after the Prisma findUnique call — it destructured the result directly. " +
      "The test expected the standard 404 envelope. Fix: added an explicit null guard " +
      "and a 404 response before destructuring.",
  },
  {
    stage: "4 — Tests with coverage (coverage gate)",
    file: "src/routes/labels.ts",
    issue: "Coverage threshold not met: functions 66.7 % < 100 % (per-file threshold)",
    detail:
      "vitest --coverage reported that the error-handling branch of PATCH /labels/:id " +
      "had zero coverage — the AI only generated a happy-path test. " +
      "The per-file threshold of functions: 100 in vitest.config.ts blocked the pipeline. " +
      "Fix: added a test for the 404 case and a test for the 500 error path.",
  },
];

// Why coverage gates matter specifically for AI-generated code
// --------------------------------------------------------------
// AI assistants are optimised for token-efficient output. This has a systematic
// side effect: generated code overwhelmingly covers the happy path. Error
// branches (null returns, constraint violations, unexpected service errors) are
// written just often enough to look complete, but are rarely exercised.
//
// The concrete pattern observed in labels.ts:
//   - Every route had a 200/201/204 success test.
//   - Zero routes had a test for the database-error (500) path.
//   - The PATCH route had no test for the 404 case.
// Branch coverage (65 %) and function coverage (100 % per-file) together
// catch both gaps: branch coverage detects untested conditional arms inside a
// function; function coverage detects entire handlers that were never called
// in any test.
//
// Without these thresholds the missing error tests would have merged silently.
// A real 500 error in production would have returned an unformatted stack trace
// instead of the JSON error envelope every other endpoint returns — inconsistent
// behaviour that is impossible to handle reliably in the frontend.
//
// The per-file override for src/routes/labels.ts (statements: 80, functions: 100)
// sets a higher bar for newly added route files than for legacy code. The intent
// is that any endpoint file added in the future — whether AI-generated or hand-
// written — must ship with near-complete function coverage before it can merge.
